...
const getPpl = async () => {
    let retrievePpl = await fetch("https://fakerapi.it/api/v1/persons?_quantity=30&_gender=male&_birthday_start=2005-01-01")
    let formattedPpl = await retrievePpl.json();
    // console.log(formattedPpl)
    let retrievePhoto = await fetch("https://fakerapi.it/api/v1/users?_quantity=30&_gender=male")
    ...
    for (let person of formattedPpl.data) {
        ...
        const personFullName = "Name: " + person.firstname + " " + person.lastname;
